...
def get_recommended_events_for_user():
    ...
    events_by_type = [event for event in events if event["event_type"] == input_event["event_type"]]
    ...
    closest_desc = difflib.get_close_matches(input_desc, event_descs, n=1, cutoff=0.8)
    
    # If no events with a similar description, return None
    if len(closest_desc)>0:
        closest_event = [event for event in events_by_type if event["eventDesc"] == closest_desc[0]]
    else:
